...
def load_from_files(F):
	"""Loads a previously saved instance of the fortirest class from pickle files.
	If we can't find/read them for some reason, just return (F, None) and GET them instead
	If everything loads ok, return (F, True)
	
	Input:
	F - The fortirest class instance
	...
	for name in F.dataframe_names:
		try:
			...
			print "Error reading file {}: {}\nReading firewalls instead\n\n".format(name+'.pkl', e)
			return F, None

	F.last_updated = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
	with open('last_updated.pkl', 'r') as f:
		F.last_updated = pickle.load(f)
	print "Successfully loaded data\n\nData is accurate as of {}".format(F.last_updated)
	ans = False
	while not ans:
		ans = raw_input("Would you like to update all the data? [y/n]: ")
		if ans.lower() == 'y':
			return (F, None)
		elif ans.lower() == 'n':
			return F, "OK"
			...
			ans = False
			print("Cmon, its either yes (y) or no (n)\n")

def get_data(F):
	"""Goes and connects to the firewalls to get data from them with the API
	TODO
	Allow user to specify specific dataframes, fields, firewalls so we don't have to get everything everytime
	"""
	# Create SCP instance to read config
	scp = ConfigParser.SafeConfigParser()
	scp.read(F.config_file)
	print "\nThis is going to take a minute or two. I am working on speeding this up.\n"
	for fwname in scp.sections():
		print "Parsing everyting from {}".format(fwname)
		F.parse_all()
	for name in F.dataframe_names:
		tmp = getattr(F, name)
		tmp.to_pickle(name+'.pkl')
	F.last_updated = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
	with open('last_updated.pkl', 'w') as f:
		pickle.dump(F.last_updated, f)
	return F, "OK"
...
def main():
	...
	if load:
		#try:
		F, status = load_from_files(F)
		#except Exception as e:
		#	print "Could not load firewall data: {}".format(e)
		#		status = None	
	if not status:
		try:
			F, status = get_data(F)
		except Exception as e:
			print "Could not GET firewall data: {}".format(e)
